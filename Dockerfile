...
RUN apk add build-base perl-dev gcc make perl-mime-base64 perl-xml-simple perl-digest-hmac perl-mime-tools perl-http-message
...
RUN cd Time-Piece-1.31 && perl Makefile.pl && make && make test && make install
RUN cd ../Net-AWS-SES-0.04 && perl Makefile.pl && make && make test && make install
